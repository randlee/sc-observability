...
def candidate_source() -> str:
    return (
        ...
        "fn main() { let service = ServiceName::new(\"bp2-candidate\").unwrap(); let config = LoggerConfig::default_for(service.clone(), PathBuf::from(\"logs\")); let (logger, mut owner) = Logger::new_with_level_owner(config).unwrap();\n"
