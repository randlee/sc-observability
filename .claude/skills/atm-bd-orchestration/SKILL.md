...
| --- | --- | --- |
| one active task per agent | dev-sanity and quality-mgr start every open task at once (`atm task start`, then `bd update --claim`) and close each as its verdict is ready; quality-mgr also closes tasks it could not start | several active tasks for a coordinator role |
