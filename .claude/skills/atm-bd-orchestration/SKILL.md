...
| --- | --- | --- |
| one active task per agent | dev-sanity follows the concurrent coordinator exception in `docs/team-protocol.md` (claim marks the check running; ATM start/close pair one at a time as verdicts arrive); quality-mgr claims every bead and closes tasks it could not start | several active tasks for a coordinator role |
