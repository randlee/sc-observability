# Step 3 — Sprint Scope Hardening (`plan_author`)

Route this step to the same `plan_author` used in Step 1 — the agent who
authored the plan being hardened. It is not always `cobs`.
...
atm send "$PLAN_AUTHOR" --stdin < /tmp/step-3-message.xml
```
...
- `FAIL` -> re-render and re-send Step 3 to `plan_author`
