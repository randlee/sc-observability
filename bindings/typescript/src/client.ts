...
function envelopeFailure(value: unknown, field = "response"): Failure {
  if (isFailure(value) && isRecord(value) && typeof value.code === "string" && typeof value.message === "string") return value;
...
class Client implements ObservabilityClient {
  ...
  private recordSuccess(outcome: ClientOutcome): void {
    this.lastResult = ok(outcome);
  }

  private recordFailure(failure: Failure): void {
    this.counts[failure.kind] = increment(this.counts[failure.kind]);
    this.lastFailure = failure;
    this.lastResult = err(failure);
  }
}
