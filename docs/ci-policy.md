...
PR target takes precedence over source branch. PRs into `develop` or
`integrate/*` run strict public API governance and complete platform
qualification, even when their source is `sprint/*` or `fix/*`. Intermediate
sprint/fix PRs retain API diff artifacts in report mode. Explicit dispatch,
reusable release qualification and non-PR runs retain complete qualification.
Phase-ending QA still runs strict `just public-api` and `just validate`.
...
| CI: `public-api-governance` | Integration reviewer; visible API diffs, report-only for intermediate PRs | Phase D missing scoped approvals before integration ownership closes | Integration no longer needs intermediate API reports |
