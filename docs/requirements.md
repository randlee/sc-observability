...
- TYP-032 `sc-observability-types` shall own the stable historical/follow query contracts: `LogQuery`, `LogOrder`, `LogFieldPredicate`, and `LogFieldMatch`.
...
- TYP-035 `sc-observability-types` shall own `QueryError` with variants `InvalidQuery`, `IoError`, `DecodeError`, `Unavailable`, and `Shutdown`.
...
- TYP-038 `sc-observability-types` shall own `TelemetryHealthProvider` as the shared telemetry-health trait used across crate boundaries.
