...
invent a competing opaque-error API. D.3 also owns migration of the existing
`LogSink` implementations in `sinks.rs` to the canonical error signature;
that narrowly scoped implementation work is excluded from D.4's call-site
migration. `examples/custom-sink-example` remains D.17's fence. D.18 alone
...
Consume obs-d-13's frozen concrete `TypedLogSink` signature specification and obs-d-12's canonical error/registry artifact. In wave 2, bind the resulting errors and codes only in owned `builder.rs`; D.3 does not edit `typed.rs` and D.18 activates its exports.
