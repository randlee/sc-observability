...
The Windows runner annotations record communication loss with no uploaded runtime evidence. The `71215ca` annotation is from run `35232045864`, job `105239201036`; its logs endpoint returned HTTP 404. These annotations establish failed qualification attempts, not an application-level cause.
