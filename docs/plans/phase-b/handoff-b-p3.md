...
| QA-B015 | Fixed: runtime contract now distinguishes crate-private core owner lifecycle from bridge completion; bridge Failed/unconfirmed does not claim core stopped. Pinned BTIT `BridgeLifecycle` is exactly Running, ShuttingDown, ShutdownTimedOut, Stopped, so no source behavior is attributed to the proposed Failed state. |
