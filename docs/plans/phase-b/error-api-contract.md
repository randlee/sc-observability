...
```rust
// Generated by the private impl_failure_builders! declarative macro for all nine types.
...
B.1a implements a private `macro_rules! impl_failure_builders` in the neutral
error module and invokes it once for each new failure type. It generates the
four identical consuming builders (`cause`, `docs`, `detail`, `source`), modifying
the existing boxed context without reallocating it or changing the stored kind.
Family-specific enums, classification tables and named constructors remain
explicit. The macro is not exported and adds no public helper symbol. Run every
builder through all nine families, asserting stored kind, diagnostic data and
context identity; test source-chain retention as well as compile-time signatures.
