...
| queue, owner, and level concurrency | `try_log_reports_queue_full_on_saturated_queue` (legacy and typed assertions), `level_owner_changes_only_its_logger_and_filters_with_shared_admission` (typed Accepted after elevation and Filtered after reset), `admission_and_level_mutation_contend_on_one_control_state` (the same synchronized contention path for legacy and typed admission) |
| flush, timeout, and shutdown | `flush_failures_propagate_and_are_counted_in_health` (legacy and typed assertions), `disconnected_writer_returns_legacy_and_typed_admission_and_flush_failures`, `shutdown_records_join_timeout_but_waits_for_join`, `emit_path_remains_available_during_maintenance_pass` |
| built-in sink, maintenance, and fault behavior | `file_and_console_fan_out_both_receive_event`, `built_in_write_failures_mark_sink_health_for_legacy_and_typed_calls`, `console_writer_failures_preserve_legacy_and_typed_write_parity`, `maintenance_failure_uses_maintenance_error_code`, retained fault-injector tests under `--all-features` |
