...
| Sprint plans | The 17 sprint records linked from the phase index | One authoritative deliverable/acceptance/validation list per record |
...
| Architecture and ADRs | [Architecture](../../architecture.md), ADR index in §7 | ADR-011–014 stay proposed until accepted |
...
| TypeScript client/exporter and Tauri adapter | PHB-010–012 | ADR-014; B.3a client and command signatures | B.3a client, host adapter and real IPC example |
| Python extension and Rust embedding rlib | PHB-010/011/013 | ADR-014; B.4 host/lifecycle signatures | B.4 runtime; B.4a platform distributions; B.5 integration; B.6 optional async |
