...
consume them, with publication deferred beyond Phase B; then add TypeScript and
...
| B.P2 | Staged/qualified core capability with two-leg consumer proof; publication deferred | [Prerequisite qualification](sprint-b-p2-runtime-publish.md) |
...
| B.2 | Staged/qualified Rust bridge and macros with candidate-consumer proof; publication deferred | [Rust qualification](sprint-b-2-publish-rust.md) |
...
| B.7 | Final Phase-B readiness inventory and registry-proof preparation; publication requires later authorization | [Binding publication](sprint-b-7-publish-bindings.md) |
...
copy of that accepted implementation; live registry proof is deferred beyond
Phase B;
all foreseeable bridge API changes are implemented in BTIT before migration.
B.2 qualifies/stages the companion pair; publication follows a later
sc-publish migration and separate authorization.
...
migration; publication follows a later sc-publish migration and separate
authorization.
...
| B.P2 must_follow B.P1 | Qualify and stage the tested core capability; publication is deferred |
...
| B.2 must_follow B.1e | Qualify and stage the improved core API and warning-only compatibility path with the companion release; publication is deferred |
...
| B.7 must_follow B.6 | Complete Phase-B readiness inventory and registry-proof preparation; live publication follows later authorization |
...
The eventual registry-dependent acceptance test waits for the separately authorized published
...
QA acceptance or later live publication -- those are separate,
subsequent gates, not development work, and are tracked in each sprint's own
handoff rather than blocking `status`.

B.7's registry-readiness tail (live publication is deferred beyond Phase B;
registry-consumer proof preparation remains) is owner-deferred until after the
Phase C `sc-publish` migration and separate authorization; its review-readiness scope (manifest, validation scripts and
